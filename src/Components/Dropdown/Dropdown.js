...
function Dropdown({ name, dropvalues, onChange, touched, errors, value_select, inputbox=false, ...props }) {
    ...
    const input_submit = async (e, name) => {
        ...
        if(name === 'Category_id') {
            if(Status) {
                await axios.post('https://storecontrolserver.herokuapp.com/category/new', { Category_id: uuidv4(), nombre: inputText})
                    .then(async (item) => {
                        category(item.data)
            ...
            de.push(inputText)
            await axios.put('https://storecontrolserver.herokuapp.com/deposito/employee', { Deposito_id: deposit.Deposito_id,  Employee_list: JSON.stringify(de)})
                .then(async (item) => {
                    console.log(de)
                    var dep = {
                    ...
                    DepositoLog(dep)
                    await axios.get('https://storecontrolserver.herokuapp.com/deposito')
                        .then(item => {
                            deposito(item.data)
                            setInputText('')
                        })
            ...
            if(Status) {
                await axios.post('https://storecontrolserver.herokuapp.com/expensecat/new', {nombre: inputText}).then(async (item)=>{
        ...
        //     await axios.post('https://storecontrolserver.herokuapp.com/deposito/new', { nombre: inputText})
        //         .then((item) => {
        //             deposito(item.data)
        //             setInputText('')
        //         })
    ...
    const category_remove = async (cate, index) => {
        ...
        category(filtered)
        if(Status) {
            await axios.delete(`https://storecontrolserver.herokuapp.com/category/delete/${CategoryAdd.filter(function(el, i) { return index === i; })[0].Category_id}`)
        } else {
            if(window.desktop) {
